...
if 'df' not in st.session_state:
    ...
    # 数値整形（世帯数の処理）
    df['世帯数'] = pd.to_numeric(
        df['世帯数'].astype(str).str.replace(',', '', regex=False),
        errors='coerce'
    ).fillna(0).astype(int)
...
if uploaded_file:
    if uploaded_file.name.endswith('.csv'):
        ...
        encoding_guess = encoding_guess if encoding_guess else 'shift-jis'
    ...
    # 数値整形（世帯数の処理）
    new_df['世帯数'] = pd.to_numeric(
        new_df['世帯数'].astype(str).str.replace(',', '', regex=False),
        errors='coerce'
    ).fillna(0).astype(int)
...
if search_town:
    filtered_df = st.session_state.df[
        st.session_state.df['住所（スプレッドシート用）'].str.contains(search_town, na=False)
    ]
